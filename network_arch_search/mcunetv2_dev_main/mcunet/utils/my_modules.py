...
class MyModule(nn.Module):
    """
    MyModule is a base class derived from nn.Module and uses Python's ABC (Abstract Base Class) mechanism to
    enforce that any subclass implements the forward, module_str, config, and build_from_config methods.
    While these methods raise a NotImplementedError in MyModule, the error will not be thrown directly here but
    will occur if the methods are called and not implemented in a child class.
    """
    def forward(self, x):
        raise NotImplementedError
